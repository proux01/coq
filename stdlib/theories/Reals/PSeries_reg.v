...
Proof.
  ...
  pose proof (Rabs_pos x) as Rabs_pos_x.
  ...
  { intro; split.
    ...
    lra. }
  exists l.
  assert (forall n:nat, 0 <= Bn n). {
    intro; apply Rle_trans with (Rabs (An n)).
    { apply Rabs_pos. }
    unfold An; apply H4; unfold Boule; simpl;
      rewrite Rminus_0_r; lra.
  }
  unfold Un_cv in H3; unfold Un_cv; intros.
  elim (H3 _ H6); intros.
  exists x0; intros.
  ...
  - destruct (Ball_in_inter c c d d z bz bz) as [delta Pd].
    assert (dz :  0 < Rmin delta (Rabs (z - x))). {
      now apply Rmin_glb_lt;[apply cond_pos
                            | apply Rabs_pos_lt; intros zx0; case xnz;
    ...
    1:apply continuity_pt_minus.
    1:apply derivable_continuous_pt; eapply exist; apply dff'; assumption.
    1:apply continuity_pt_const; intro; intro; reflexivity.
    1:apply continuity_pt_minus;
    ...
    - destruct (Ball_in_inter c c d d x bx bx) as [delta Pdelta].
      ...
      + apply Rle_trans with (1 := Rdist_tri _ _ (rho_ p (y + Rmin (Rmin d' d2) delta/2))).
        ...
        * apply Rlt_le.
          ...
          1:replace (rho_ p (y + Rmin (Rmin d' d2) delta / 2)) with
