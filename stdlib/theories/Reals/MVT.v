...
Proof.
  ...
  assert (X:forall c:R, a < c < b -> derivable_pt h c). {
    intros;
      change
  ...
  assert (forall c:R, a <= c <= b -> continuity_pt h c). {
    intros; unfold h;
      change
  ...
  assert (H4 := continuity_ab_maj h a b H2 H3).
  assert (H5 := continuity_ab_min h a b H2 H3).
  elim H4; intros Mx H6.
  elim H5; intros mx H7.
  assert (h a = h b) by (unfold h;ring).
  set (M := h Mx); set (m := h mx).
  assert
    (forall (c:R) (P:a < c < b),
      derive_pt h c (X c P) =
      (g b - g a) * derive_pt f c (pr1 c P) -
  ...
  1:case (Req_dec (h a) m); intro.
  - (*** h constant ***)
    assert (forall c:R, a <= c <= b -> h c = M). {
...
Proof.
  ...
  1:intro; cut (forall c:R, a <= c <= b -> derivable_pt id c).
  1:intro X1; cut (forall c:R, a < c < b -> derivable_pt id c).
  1:intro X2; cut (forall c:R, a <= c <= b -> continuity_pt id c).
  - intro; elim (MVT f id a b X0 X2 H H1 H2); intros x (P,H3).
    exists x; split.
    + cut (derive_pt id x (X2 x P) = 1).
      1:cut (derive_pt f x (X0 x P) = f' x).
...
Proof.
  ...
  decompose [and] H8; intros; generalize (H7 (delta / 2) H9 H12);
    assert ((f (x + delta / 2) - f x) / (delta / 2) <= 0). {
  ...
  assert (0 < - ((f (x + delta / 2) - f x) / (delta / 2) - l)) by lra.
  unfold Rabs;
    case (Rcase_abs ((f (x + delta / 2) - f x) / (delta / 2) - l)) as [Hlt|Hge]; lra.
Qed.
...
Proof.
  intros.
  assert (forall h:R, - - f h = f h) by (intros;ring).
  ...
  apply H1.
  2:{ assumption. }
  assert (forall x:R, 0 <= derive_pt (- f) x (derivable_opp f pr x)). {
    intro.
    assert (H3 := derive_pt_opp f x0 (pr x0)).
    assert
      (derive_pt (- f) x0 (derivable_pt_opp f x0 (pr x0)) =
         derive_pt (- f) x0 (derivable_opp f pr x0)) by apply pr_nu.
    rewrite <- H4.
    rewrite H3.
...
Proof.
  intros.
  assert (X:derivable (g - f)) by (apply derivable_minus; assumption).
  assert (forall c:R, a <= c <= b -> derive_pt (g - f) c (X c) <= 0). {
    intros.
    assert
      (derive_pt (g - f) c (X c) =
         derive_pt (g - f) c (derivable_pt_minus _ _ _ (pr2 c) (pr1 c))) by apply pr_nu.
    rewrite H2.
