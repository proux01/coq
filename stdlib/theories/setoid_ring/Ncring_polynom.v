...
Proof.
  ...
  - assert (Hh := ring_morphism_eq  c 0).
    ...
    intros.
    rewrite Hh.
    + rewrite ring_morphism0. now rewrite_db rsimpl.
    + apply Ceqb_eq. trivial.
  - destruct (Pos.compare_spec i p).
    + subst. assert (Hh := @Peq_ok P3 P0). case_eq (P3=? P0).
      * intro. simpl.
        rewrite Hh.
        -- rewrite Pos.add_comm, pow_pos_add, Pphi0.
           now rewrite_db rsimpl.
        -- trivial.
      * intros. reflexivity.
    + reflexivity.
    + reflexivity.
...
Proof.
  induction P as [|????? IH2]; simpl; intros.
  - now rewrite ring_morphism_add.
  - now rewrite IH2, !(ring_add_comm [_]), ring_add_assoc.
Qed.

Lemma PmulC_aux_ok : forall c P l, (PmulC_aux P c)@l ==  P@l * [c].
Proof.
  induction P as [|? IH1 ??? IH2]; simpl; intros.
  - now rewrite ring_morphism_mul.
  - rewrite mkPX_ok, IH1, IH2, !phiCR_comm.
    now rewrite_db rsimpl.
Qed.
...
Proof.
  ...
  assert (Hh:= ring_morphism_eq c 0);case_eq (c =? 0).
  - intros. rewrite Hh.
    + now rewrite Pphi0, ring_morphism0, ring_mul_0_r.
    + now apply Ceqb_eq.
  - assert (H1h:= ring_morphism_eq c 1);case_eq (c =? 1);intros.
    + rewrite H1h.
      * now rewrite ring_morphism1, ring_mul_1_r.
      * now apply Ceqb_eq.
    + apply PmulC_aux_ok.
Qed.
...
Proof.
  induction P as [|? IH1 ??? IH2];simpl;intros.
  - now rewrite ring_morphism_opp.
  - rewrite IH1, IH2.
    now rewrite ring_opp_add, ring_opp_mul_l.
Qed.
...
Proof.
  induction P2 as [|? IH1 ??? IH2];simpl;intros.
  - split.
    + intros. apply PaddCl_ok.
    + intros P. induction P as [|? IH'1 ??? IH'2].
      * unfold PaddX. intros. now rewrite mkPX_ok.
      * intros. simpl.
        destruct (Pos.compare_spec k p) as [Hh|Hh|Hh].
        -- destr_pos_sub H1h; subst.
           ++ rewrite mkPX_ok, PaddCl_ok.
              now rewrite_db rsimpl.
           ++ rewrite mkPX_ok, IH'1, Pos.add_comm, pow_pos_add.
              now rewrite_db rsimpl.
           ++ rewrite mkPX_ok, PaddCl_ok, mkPX_ok, Pphi0, Pos.add_comm, pow_pos_add.
              now rewrite_db rsimpl.
        -- rewrite mkPX_ok, IH'2.
           rewrite_db rsimpl. now rewrite (ring_add_comm (_ * _)).
        -- assert (H1h := ring_morphism_eq c 0);case_eq (Ceqb c  0);
             intros; simpl.
           ++ rewrite H1h;trivial.
              ** rewrite ring_morphism0. now rewrite_db rsimpl.
              ** apply Ceqb_eq; trivial.
           ++ reflexivity.
  - decompose [and] IH1. decompose [and] IH2. clear IH1 IH2.
    split.
    + intros. rewrite H0. rewrite H1.
      now rewrite ring_add_assoc.
    + induction P.
      * unfold PaddX. intros. rewrite mkPX_ok. reflexivity.
      * intros. rewrite PaddXPX.
        destruct (Pos.compare_spec k p1) as [H3h|H3h|H3h].
        -- destr_pos_sub H4h; subst.
           ++ rewrite mkPX_ok. simpl. rewrite H0, H1.
              now rewrite_db rsimpl.
           ++ rewrite mkPX_ok, IHP1. simpl.
              rewrite Pos.add_comm, pow_pos_add.
              now rewrite_db rsimpl.
           ++ rewrite mkPX_ok. simpl. rewrite H0, H1. rewrite mkPX_ok, Pphi0.
              rewrite Pos.add_comm, pow_pos_add.
              now rewrite_db rsimpl.
        -- rewrite mkPX_ok. simpl. rewrite IHP2.
           now rewrite ring_add_comm, <- ring_add_assoc, (ring_add_comm (_ @ _)).
        -- now rewrite mkPX_ok.
Qed.
...
Proof.
  unfold Psub. intros. now rewrite Padd_ok, Popp_ok, ring_sub_def.
Qed.

Lemma Pmul_ok : forall P P' l, (P**P')@l == P@l * P'@l.
Proof.
  induction P'; simpl; intros.
  - rewrite PmulC_ok. reflexivity.
  - rewrite PaddX_ok. rewrite IHP'1. rewrite IHP'2.
    now rewrite_db rsimpl.
Qed.
...
Proof.
  intros. simpl. rewrite ring_morphism0, ring_morphism1. now rewrite_db rsimpl.
Qed.
...
Proof.
  ...
  induction p;simpl;intros.
  - rewrite subst_l_ok, !Pmul_ok, !IHp. now rewrite_db rsimpl.
  - rewrite Pmul_ok, !IHp. now rewrite_db rsimpl.
  - now rewrite subst_l_ok, Pmul_ok.
Qed.
...
Proof.
  destruct n;simpl.
  - reflexivity.
  - rewrite Ppow_pos_ok; trivial.
    now rewrite Pphi1, ring_mul_1_r.
Qed.
...
Proof.
  intros.
  induction pe as [ | | | |pe1 IH1 pe2 IH2|pe1 IH1 pe2 IH2|pe1 IH1 pe2 IH2|pe IH|pe IH n]; simpl.
  - now rewrite <- ring_morphism0.
  - now rewrite <- ring_morphism1.
  - reflexivity.
  - rewrite ring_morphism0, ring_morphism1. now rewrite_db rsimpl.
  - rewrite IH1, IH2. unfold Psub.
    destruct pe2; now rewrite Padd_ok.
  - rewrite IH1, IH2. unfold Psub.
    now rewrite Padd_ok, Popp_ok, ring_sub_def.
  - now rewrite IH1, IH2, Pmul_ok.
  - now rewrite IH, Popp_ok.
  - rewrite Ppow_N_ok; [|intros; reflexivity].
    rewrite rpow_pow_N; [|now apply pow_th].
    destruct n; simpl; [now rewrite ring_morphism1|].
    induction p as [p IHp|p IHp|]; simpl; [| |now apply IH].
    + now rewrite IHp, IH, !Pmul_ok.
    + now rewrite Pmul_ok, IHp.
Qed.

Lemma norm_subst_spec : forall l pe, PEeval l pe == (norm_subst pe)@l.
Proof.
  intros. apply norm_aux_spec.
Qed.
