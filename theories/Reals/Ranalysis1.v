...
Proof.
  ...
  - cut (n = 0%nat \/ (0 < n)%nat).
    + intro; elim H0; intro.
      * rewrite H1; simpl.
        replace (fun y:R => y * 1) with (id * fct_cte 1)%F by reflexivity.
        replace (1 * 1) with (1 * fct_cte 1 x + id x * 0).
        -- apply derivable_pt_lim_mult.
           ++ apply derivable_pt_lim_id.
           ++ apply derivable_pt_lim_const.
        -- unfold fct_cte, id; ring.
      * replace (fun y:R => y ^ S n) with (fun y:R => y * y ^ n) by reflexivity.
        replace (pred (S n)) with n; [ idtac | reflexivity ].
        replace (fun y:R => y * y ^ n) with (id * (fun y:R => y ^ n))%F by reflexivity.
        set (f := fun y:R => y ^ n).
...
Proof.
  ...
  { (* 0 < _ *) assert (H3 := derivable_derive f c pr).
    ...
    cut (0 < (b - c) / 2).
    1:intro; cut (Rmin (delta / 2) ((b - c) / 2) <> 0).
    1:intro; cut (Rabs (Rmin (delta / 2) ((b - c) / 2)) < delta).
    - intro.
      assert (H11 := H7 (Rmin (delta / 2) ((b - c) / 2)) H9 H10).
      cut (0 < Rmin (delta / 2) ((b - c) / 2)).
      1:intro; cut (a < c + Rmin (delta / 2) ((b - c) / 2)).
      1:intro; cut (c + Rmin (delta / 2) ((b - c) / 2) < b).
      1:intro; assert (H15 := H1 (c + Rmin (delta / 2) ((b - c) / 2)) H13 H14).
      1:cut ((f (c + Rmin (delta / 2) ((b - c) / 2)) - f c)
               / Rmin (delta / 2) ((b - c) / 2) <= 0).
      1:intro; cut (- l < 0).
      1:intro; unfold Rminus in H11.
      1:cut ((f (c + Rmin (delta / 2) ((b + - c) / 2)) + - f c)
               / Rmin (delta / 2) ((b + - c) / 2) + - l < 0).
      1:intro; cut (Rabs ((f (c + Rmin (delta / 2) ((b + - c) / 2)) + - f c)
  ...
  - intro; elim (H6 (- (l / 2)) H7); intros delta H9.
    cut (0 < (c - a) / 2).
    1:intro; cut (Rmax (- (delta / 2)) ((a - c) / 2) < 0).
    1:intro; cut (Rmax (- (delta / 2)) ((a - c) / 2) <> 0).
    1:intro; cut (Rabs (Rmax (- (delta / 2)) ((a - c) / 2)) < delta).
    1:intro; generalize (H9 (Rmax (- (delta / 2)) ((a - c) / 2)) H11 H12); intro;
      cut (a < c + Rmax (- (delta / 2)) ((a - c) / 2)).
    1:cut (c + Rmax (- (delta / 2)) ((a - c) / 2) < b).
    1:intros; generalize (H1 (c + Rmax (- (delta / 2)) ((a - c) / 2)) H15 H14);
      intro; cut
               (0 <= (f (c + Rmax (- (delta / 2)) ((a - c) / 2)) - f c)
                      / Rmax (- (delta / 2)) ((a - c) / 2)).
    1:intro; cut (0 < - l).
    1:intro; unfold Rminus in H13;
      cut
        (0 < (f (c + Rmax (- (delta / 2)) ((a + - c) / 2)) + - f c)
               / Rmax (- (delta / 2)) ((a + - c) / 2) + - l).
    1:intro;
      cut
        (Rabs ((f (c + Rmax (- (delta / 2)) ((a + - c) / 2)) + - f c)
                 / Rmax (- (delta / 2)) ((a + - c) / 2) + - l)
         < - (l / 2)).
    + unfold Rabs;
        case
...
Proof.
  ...
  - elim H2; intro.
    ...
    + assert (H4 := derive_pt_eq_1 f x l (pr x) H1).
      cut (0 < - (l / 2)).
      1:intro; elim (H4 (- (l / 2)) H5); intros delta H6.
      1:cut (delta / 2 <> 0 /\ 0 < delta / 2 /\ Rabs (delta / 2) < delta).
      1:intro; decompose [and] H7; intros; generalize (H6 (delta / 2) H8 H11);
      cut (0 <= (f (x + delta / 2) - f x) / (delta / 2)).
      1:intro; cut (0 <= (f (x + delta / 2) - f x) / (delta / 2) - l).
      * intro; unfold Rabs;
