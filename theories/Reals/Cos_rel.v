...
2:{ apply sum_eq; intros.
    ...
    { replace ((-1) ^ i) with ((-1) ^ i0 * (-1) ^ (i - i0)).
      - ring.
      - pattern i at 2; replace i with (i0 + (i - i0))%nat by nia.
        rewrite pow_add.
        ring. }
    unfold C.
    ...
    rewrite Rmult_1_l.
    rewrite Rinv_mult.
    replace (2 * i - 2 * i0)%nat with (2 * (i - i0))%nat by lia.
    reflexivity. }
pose
 (sin_nnn :=
  fun n:nat =>
...
- rewrite <- sum_plus.
  ...
  2:{ apply sum_eq; intros.
      unfold Wn.
      apply Rmult_eq_compat_l.
      replace (2 * S i - 2 * i0)%nat with (2 * (S i - i0))%nat by lia.
      reflexivity. }
  replace
    (sum_f_R0
       (fun l:nat =>
  ...
  unfold Wn.
  apply Rmult_eq_compat_l.
  replace (2 * S i - S (2 * i0))%nat with (S (2 * (i - i0))) by lia.
  reflexivity.
- match goal with
    |- _ = - ?r =>
  ...
  rewrite decomp_sum.
  2:nia.
  replace (sin_nnn 0%nat) with 0 by reflexivity.
  rewrite Rplus_0_l.
  ...
  replace (2 * S i - (2 * i0 + 1))%nat with (2 * (i - i0) + 1)%nat by lia.
  reflexivity.
Qed.
