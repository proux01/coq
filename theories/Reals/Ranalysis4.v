...
Proof.
  ...
  assert (N = 0%nat \/ (0 < N)%nat) by nia.
  elim H0; intro.
  { rewrite H1.
    simpl.
    change (fun y:R => An 0%nat * 1 + An 1%nat * (y * 1)) with
    ...
    - apply derivable_pt_lim_scal.
      ...
      + apply derivable_pt_lim_const. }
  change (fun y:R => sum_f_R0 (fun k:nat => An k * y ^ k) (S N)) with
    ((fun y:R => sum_f_R0 (fun k:nat => (An k * y ^ k)%R) N) +
       (fun y:R => (An (S N) * y ^ S N)%R))%F.
  ...
  2:{ assert (pred (S N) = S (pred N)) by lia.
      rewrite H2.
      rewrite tech5.
  ...
  change (fun y:R => An (S N) * y ^ S N) with
  (mult_real_fct (An (S N)) (fun y:R => y ^ S N)).
  apply derivable_pt_lim_scal.
