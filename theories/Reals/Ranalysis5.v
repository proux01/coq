...
Proof.
  ...
  { intros a b fa_eq_fb.
    pose proof (f_incr a b);pose proof (f_incr b a);lra. }
  apply f_inj,Hyp.
Qed.
(* end hide *)
...
Proof.
  assert (Sublemma : forall x y lb ub, lb <= x <= ub /\ lb <= y <= ub -> lb <= (x+y) / 2 <= ub) by (intros;lra).
...
Proof.
  ...
  assert (Un_cv Vn x0) by (unfold Vn; assumption).
  intros.
  assert (Temp : x <= x0 <= y).
  { apply IVT_interv_prelim1 with (D:=(fun z : R => cond_positivity (f z))) ; assumption. }
...
Proof.
  ...
  case_le y_encad1 ; intros y_encad2 y_encad3 ;
    case_le y_encad3.
  - intro y_encad4.
...
Proof.
  ...
  unfold comp in f_eq_g. rewrite f_eq_g.
  2:{ split. { apply Rle_trans with (r2:=x1) ; intuition. }
    apply Rle_trans with (r2:=x2) ; intuition. }
  rewrite f_eq_g.
  2:assumption.
  unfold id.
...
Proof.
  ...
  2:{ field ; split.
      ...
      { rewrite f_eq_g in Hfin.
        { unfold id in Hfin. exact Hfin. }
        assumption. }
      assumption. }
  assert (Hrewr : h = (comp f g ) (x+h) - (comp f g) x).
  { rewrite f_eq_g.
    { rewrite f_eq_g.
      { unfold id ; rewrite Rplus_comm ;
...
Proof.
  intros lb_lt_ub x_encad local_recip Df_neq.
...
Proof.
  intros c' r xinb Dfn_eq_fn' fn_CV_f fn'_CVU_g g_cont eps eps_pos.
  ...
  { assert (pr1 : forall c : R, x + h < c < x -> derivable_pt (fn N) c).
    ...
    2:{ assert (H := pr1 c P) ; elim H ; clear H ; intros l Hl.
